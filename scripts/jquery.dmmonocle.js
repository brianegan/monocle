...
$.fn.dmMonocle = function(options) {
  ...
  return this.each(function() {
	...
	******************************************/	
	...
	devUrlPrefix = o.urlPrefix,
	bigWidth = o.dmImageWidth,
	bigHeight = o.dmImageHeight,
	CISOPTR = o.CISOPTR,
	CISOROOT = o.CISOROOT,
	thumbWidth,
	thumbHeight,
	...
	******************************************/
	function buildImage(lvlZoom, lvlRotation, offsetRatioX, offsetRatioY) {
		...
		if (hideNav === true) {
			$('<div id="dmThumbnail"></div>').appendTo($this).css('top', ((thumbHeightMax + 10) * -1) + 'px');
		} else {
			$('<div id="dmThumbnail" style="left:0; top:0"></div>').appendTo($this);	
		}
		$('<div id="dmMainImageContainer"></div>').appendTo($this);
		...
		if (imageWidth <= viewerWidth && imageHeight <= viewerHeight) {						
			...
			$(thumbImage)
				.load(function () {
					...
					mainImageBG = (o.searchText.length > 0) ? 
						"url(" + devUrlPrefix + "/cgi-bin/getimage.exe?CISOROOT=" + CISOROOT + "&CISOPTR=" + CISOPTR + "&DMSCALE=" + (lvlZoom * 100) + "&DMWIDTH=" + dmWidth + "&DMHEIGHT=" + dmHeight + "&DMROTATE=" + lvlRotation + "&DMTEXT=" + o.searchText + ")" : 
						"url(" + devUrlPrefix + "/cgi-bin/getimage.exe?CISOROOT=" + CISOROOT + "&CISOPTR=" + CISOPTR + "&DMSCALE=" + (lvlZoom * 100) + "&DMWIDTH=" + dmWidth + "&DMHEIGHT=" + dmHeight + "&DMROTATE=" + lvlRotation + ")";
	...
	******************************************/
	function buildMenu(initZoomLevel, initRotationLevel) {
		...
		*******************************/
		...
		// Build the Menu Div
		$('<div id="dmMonocleMenu"></div>').insertBefore($this);
	...
	******************************************/
	...
	function dblClickMove(xPos, yPos) {
		...
		if ((xPos - $this.offset().left) > (viewerWidth / 2)) {
			var mainLeft = (mainTempLeft + viewerWidth / 2) - (xPos - $this.offset().left);					
		...
		} else if ((xPos - $this.offset().left) < (viewerWidth / 2)) {
			mainLeft = mainTempLeft + (viewerWidth / 2 - (xPos - $this.offset().left));
		...
		if ((yPos - $this.offset().top) > (viewerHeight / 2)) {
			var mainTop = (mainTempTop + viewerHeight / 2) - (yPos - $this.offset().top);
		...
		} else if ((yPos - $this.offset().top) < (viewerHeight / 2)) {
			mainTop = mainTempTop + (viewerHeight / 2 - (yPos - $this.offset().top));
	...
	******************************************/
	function dblClickZoom(xScrollPos, yScrollPos, scrollZoomLvl) {
		...
		var zoomOffsetRatioX = ((defaultMainLeft - mainTempLeft) + (xScrollPos - $this.offset().left)) / (tempImageWidth * scrollZoomLvl);
		var zoomOffsetRatioY = ((defaultMainTop - mainTempTop) + (yScrollPos - $this.offset().top)) / (tempImageHeight * scrollZoomLvl);
	...
	function viewerDownloadImage() {
  	...
  	for (var i = 0; i < printSizes.length; i++) {
  		...
  		var width300DPI = widthInches * 300,
  			...
  			widthRatio300DPI = width300DPI / o.dmImageWidth * 100,
  			heightRatio300DPI = height300DPI / o.dmImageHeight * 100,
  			width72DPI = widthInches * 72,
  			height72DPI = heightInches * 72,
  			widthRatio72DPI = width72DPI / o.dmImageWidth * 100,
  			heightRatio72DPI = height72DPI / o.dmImageHeight * 100;
  		...
  		if (o.dmImageWidth < o.dmImageHeight) {				
  			if (height300DPI <= o.dmImageHeight) {					
  			...
  			if (height72DPI <= o.dmImageHeight) {
  			...
  			if (width300DPI <= o.dmImageWidth) {								
  				printLinks300DPI.push("http://cdmtest.library.unlv.edu/cgi-bin/getimage.exe?CISOROOT=" + CISOROOT + "&CISOPTR=" + CISOPTR + "&DMSCALE=" + ratio300DPI + "&DMWIDTH=" + width300DPI + "&DMHEIGHT=" + height300DPI + "&DMROTATE=0");	
  				printSizes300DPI.push(width300DPI + "x" + height300DPI);	
  			}				
  			if (width72DPI <= o.dmImageWidth) {
  				printLinks72DPI.push("http://cdmtest.library.unlv.edu/cgi-bin/getimage.exe?CISOROOT=" + CISOROOT + "&CISOPTR=" + CISOPTR + "&DMSCALE=" + ratio72DPI + "&DMWIDTH=" + width72DPI + "&DMHEIGHT=" + height72DPI + "&DMROTATE=0");
  				printSizes72DPI.push(width72DPI + "x" + height72DPI);
  			}
  		}
  	...
  	var downloadRatio = o.dmImageHeight / o.dmImageWidth;		
  	...
  	.slider({ 
  		animate: true,
  		value: (o.dmImageWidth / 2),
  		max: o.dmImageWidth,
  		min: 0
  	});

  	// Append Slider Info & Download Button
  	$("<div id='dmDownloadImagSliderVals'>" + (o.dmImageWidth / 2) + " x " + (o.dmImageHeight / 2) + "</div><p><a href='http://cdmtest.library.unlv.edu/cgi-bin/getimage.exe?CISOROOT=" + CISOROOT + "&CISOPTR=" + CISOPTR + "&DMSCALE=50&DMWIDTH=" + (o.dmImageWidth / 2) + "&DMHEIGHT=" + (o.dmImageHeight / 2) + "&DMROTATE=0' target='_blank' id='dmDownloadImageLink'>Download!</a></p>").appendTo("#dmDownloadImage");
  	...
  	$('#dmDownloadImageSlider').bind('slide', function(event, ui) { 
  		...
  		var currentZoomLevel = (currentDownloadWidth / o.dmImageWidth) * 100;
